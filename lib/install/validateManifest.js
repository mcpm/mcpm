...
let checkCompatibility = function (manifest) {
  ...
  if (!semver.satisfies(actualVersion, compatibleRange)) {
    winston.debug('install.validateManifest: incompatible mc, returning error', { actualVersion, compatibleRange })
...
let validateManifest = function (packageDirectory) {
  ...
  let invalidBasicInfo = validateBasicInfo(manifest)
  if (invalidBasicInfo) {
    return invalidBasicInfo
  }

  let compatibilityError = checkCompatibility(manifest)
  if (compatibilityError) {
    return compatibilityError
  }

  let invalidInstallFields = validateInstallFields(manifest)
  if (invalidInstallFields) {
