...
let fromFolder = function (packageDirectory, zipPath) {
  ...
  if (config.install_file_list) {
    ...
    var result = copyFiles(list, packageDirectory, zipPath, config)
  ...
  if (config.install_executable) {
    ...
    var result = invokeInstallExecutable(config.install_executable,
      packageDirectory)
  ...
  var result = util.addInstalledPackage(config.name, config.version)
